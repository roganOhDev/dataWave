...
@router.get("/tables")
def find_all_tables(connection_uuid: str, session: Session = Depends(db.session)) -> [str]:
    return composite_service.find_tables(connection_uuid, session)


@router.get("", response_model=Table_List_Dto)
def find(uuid: str, session: Session = Depends(db.session)) -> Table_List_Dto:
    return composite_service.find(uuid, session)


@router.put("")
def create(request: Table_List_Create_Dto, session: Session = Depends(db.session)) -> List[str]:
    return composite_service.create(request, session)


@router.put("/{uuid}/reset_pk")
def update(uuid: str, request: Table_List_Update_Dto, session: Session = Depends(db.session)) -> Table_List_Dto:
    return composite_service.update(uuid, request, session)


@router.patch("/{uuid}")
def update(uuid: str, pk_max: int = Query(None), session: Session = Depends(db.session)) -> Table_List_Dto:
    return composite_service.update_pk_max(uuid, pk_max, session)
